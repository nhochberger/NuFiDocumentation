...
This sectionis intended to give a brief overview of how thresholding in general
...
and white, image. This is achieved by simply comparing a pixel's (brightness)
value to a defined value, the threshold. If the pixel's value is below that
threshold, the value is set to 0 (black), if it is above the threshold, it is
set to 1 (white). Note that in some cases black and white may be switched in
...
While this is pretty much the idea behin thresholding, there are several
sophisticated approaches on how to determine a suitable value to be used as
threshold based on an image's histogram. Figure \ref{fig:example_histograms}
shows the histograms of the original channel 3 image as shown in
\ref{fig:channel3} and the brightness corrected version as shown in figure
\ref{fig:example_channel3_prepared}. Apparently, finding a suitable threshold is
much easier in the brightness corrected version of the image.
